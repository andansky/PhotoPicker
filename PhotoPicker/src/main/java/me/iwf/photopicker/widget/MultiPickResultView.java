...
public class MultiPickResultView extends FrameLayout {
    ...
    public void init(Activity context,int imgNum,@MultiPicAction  int action, ArrayList<String> photos){
        this.action = action;
        ...
        photoAdapter = new PhotoAdapter(context,imgNum, selectedPhotos);
    ...
    public void init(Activity context, Fragment fragment,int imgNum,@MultiPicAction int action, ArrayList<String> photos){
        Log.e("init","init");
        this.action = action;
        if (action == MultiPickResultView.ACTION_ONLY_SHOW){//当只用作显示图片时,一行显示3张
            recyclerView.setLayoutManager(new StaggeredGridLayoutManager(3, OrientationHelper.VERTICAL));
        ...
        photoAdapter = new PhotoAdapter(context,fragment, imgNum,selectedPhotos);
        photoAdapter.setAction(action);
        recyclerView.setAdapter(photoAdapter);
        //recyclerView.setLayoutFrozen(true);
